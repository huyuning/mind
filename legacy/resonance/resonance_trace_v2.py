...
class ResonanceTraceV2:
    ...
    def trace(self, data, error_pos, max_iter=20):
        original = self.encode(data)
        print(f"\n原始数据: {data}")
        print(f"原始码:   {original} (syn=0)")

        current = original.copy()
        current[error_pos] ^= 1

        print(f"错误位:   {error_pos}")
        print(f"错误码:   {current} (syn={self.syndrome(current)})")

        print(f"\n迭代过程:")
        seen = {tuple(original): -1, tuple(current): 0}
        history = [original, current]

        for i in range(max_iter):
            syndrome = self.syndrome(current)
            detected = self.get_error_pos(syndrome) if syndrome > 0 else None

            after, flipped = self.resonance_flip(current, detected or np.random.randint(0, 7))

            print(f"  步{i+1}: syn={syndrome}, 翻转位{detected or '随机'} → {after} (syn={self.syndrome(after)})")

            if tuple(after) in seen:
                print(f"  → 循环！状态 {after} 在步{seen[tuple(after)]}已出现")
                return i + 1

            current = after
            seen[tuple(current)] = i + 1
    ...
    def run(self):
        ...
        for data, err in tests:
            self.trace(data, err, max_iter=15)


if __name__ == "__main__":
    np.random.seed(42)
    exp = ResonanceTraceV2()
    exp.run()